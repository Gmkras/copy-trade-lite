...
### D4. Grids
Feed and detail pages: `lg:grid lg:grid-cols-[minmax(0,460px)_minmax(0,1fr)] lg:gap-8`, `lg:max-w-6xl` (1152 px). Trade: `lg:grid-cols-[minmax(0,520px)_minmax(0,1fr)]` with the account card `lg:sticky lg:top-20`. Cards keep their radius and padding; the yellow button keeps its full width inside its column. The Trade chart height comes from `useMediaQuery("(min-width: 1024px)") ? 260 : 110`.
...
- [The account card is sticky next to a long form] → `lg:self-start lg:sticky lg:top-20` with `max-h-[calc(100dvh-6rem)] overflow-y-auto` so long fills lists scroll inside the card.
