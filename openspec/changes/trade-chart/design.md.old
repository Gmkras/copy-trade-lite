...
Above the chart: "1 BTC = **$80,237**" in Space Grotesk 28 px, then "+0.4 % over the last hour" in the up/down colour, computed from the first and last close of the visible range (pure `rangeChange(candles)`, tested). The chart is 160 px on Trade so the yellow button stays above the fold at 812: title 40 + pills 60 + hero 64 + chart 160 + toolbar 36 + Up/Down 100 + How much 150 + button 56 + gaps ≈ 740. Verified by measuring the button's bottom edge in the browser, not by arithmetic.
