...
- [x] 4.4 End-to-end from the UI at 375 px: tap the yellow button once with the minimum size. Verify: busy label, then success toast with an explorer link that opens a successful transaction, and the Positions list updates within 10 s; a rapid double tap sends exactly one `POST /api/order` (check the dev log). Record the hash in the commit body.
