...
- [x] 5.1 Run the P-R review prompt on the diff against `specs/constitution.md`, with two extra questions: does any chart control lead to a write (it must not), and does the Trade screen still have exactly one yellow action. Verify: findings fixed in a separate `fix:` commit, or "no findings" recorded here.

  **P-R result (2026-09-07): no findings.** `MarketChart*`, `SignalCard`, `SignalDetail`, `lib/charts.ts` and the candles route contain no `fetch` POST, `postEnvelope` or `placeMarketOrder`; the only write in the touched files is `TradeForm`'s existing order submit, unchanged. Chart controls only change local state or issue `GET /api/candles`. On Trade the visible yellow elements are exactly one: "Buy 0.00002 BTC ≈ $1.60" (computed-colour scan). No `any`, `@ts-ignore`, empty `catch`, `NEXT_PUBLIC_` or `MAINNET` in `app/`, `components/`, `hooks/`, `lib/`, `scripts/`. Typecheck, lint, 114 tests and the production build are green. Two defects were found and fixed inside the change before this review: the Trade layout that overflowed the fold (design D6 rewritten with the measured numbers, `design.md.old` kept) and card level lines losing their names when their axis label was hidden.
