...
### D7. The builder-fee approval record moves into the database
Found during task 3.3: the deployed app refused every order with `FEE_BOUND` because `assertFeeBound` reads the approval `pnpm approve` recorded, and that record was `data/builder-approval.json` — a gitignored local file a serverless deploy never has. The approval itself is on chain, but the SDK exposes no read for it, so the app keeps its own record; it now lives in a `builder_approvals` table (keyed by subaccount and builder) in the same database as the signals. `pnpm approve` writes it, `getApprovedBuilderFee()` reads it, both async. Alternatives: an environment variable with the approval hash — rejected, it would let a deploy assert a bound it never verified; approving automatically on the first order — rejected, a signing step inside a request path is exactly what the approve-then-place design keeps out of the app.
