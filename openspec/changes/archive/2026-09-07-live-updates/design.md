...
`Feed` knows each idea's `tpPrice`/`slPrice` and now receives live prices. When a streamed price crosses a level of an **open** idea, it calls `feed.refresh()` — and the server settles the outcome from candles as it already does. The browser never writes an outcome or renders one it invented: it only shortens the wait. This keeps `settleSignal` the single judge (what is shown and what is stored cannot disagree) and it is the reason the spec scenario "The browser never invents the outcome" exists.

**Revised while implementing.** The first draft retried every 3 s per idea. Measuring showed why that is wrong: the streamed **mid** can reach a level before any trade prints there, and the server settles from candles built from trades — observed live, with the mid at 79,161 while the last 1-minute candles topped at 79,165 and the idea's take profit at 79,099.91 stayed unsettled for a while. So a crossing is a **hint, not a verdict**, and an unconfirmed one would ask again on every price tick. The retry is now **30 s per idea**: the first crossing still refreshes immediately (which is what the spec requires), and a crossing the candles have not confirmed yet costs two requests a minute instead of twenty.
...
**Two corrections the measurements forced**, both about the same thing — the route closes its own window on purpose, so reconnecting is *normal operation*, not an outage:

1. **A cadence change must not fetch.** `usePoll` ran `load()` in the same effect that armed the timer, so every switch between the live and fallback cadences fetched immediately. With a stream that reconnects on a timer, that cost three feed requests per reconnect: **38 requests in 180 s** where six were expected. The effect is now split — one fetches when the url changes, another re-arms the timer when the cadence changes — and the same window costs **3 requests in 90 s**, exactly the heartbeat.
2. **`live` needs a grace period.** `EventSource` fires `error` during the routine reconnect, so the label flickered "live → refreshing every 10s → live" every window, which reads as broken. `useLive` now waits 8 s before saying it is not live, and any event that arrives first cancels the wait. This is not a softer promise: if the stream is genuinely down the label flips and stays flipped, which is what the spec requires.
