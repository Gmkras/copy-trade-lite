...
*Implementation note (task 4.2):* `approve_max_builder_fee_for_subaccount` aborts with `EBUILDER_SUBACCOUNT_NOT_FOUND` when `builderAddr` is a plain wallet address, and succeeds when it is the builder's **primary Decibel subaccount** (the object that receives the fees). So `BUILDER_ADDRESS` must be a subaccount address; for this demo it is the app's own primary subaccount (printed by `pnpm smoke`). `.env.example` and the README say so; `smoke` warns when `BUILDER_ADDRESS` equals the wallet address.
...
`orders.ts` reads `feeBps` from the client (env constant, already `0..10`) and throws `TradeError("FEE_BOUND", ...)` if `feeBps > approvedMax || feeBps > 10` **immediately before** `placeOrder`. No function in the codebase accepts a fee as a parameter. Alternative: trust env alone — rejected; S3 asks for the check at the last point before signing.

*Implementation note (task 4.1):* neither the SDK nor the on-chain ABI exposes a **view** for the approved perp builder fee (only `approve_max_builder_fee_for_subaccount` / `revoke_…` entry functions; the sole read, `get_approved_max_spot_builder_fee`, is spot-only). So the approve script records what it approved in `data/builder-approval.json` (gitignored; `{ builderAddr, subaccountAddr, maxFeeBps, transactionHash, approvedAt }`), and `assertFeeBound` requires a record for the same builder/subaccount with `feeBps <= maxFeeBps <= 10`. No record → "run `pnpm approve` first", before any transaction is built. The chain still enforces the real bound; this check turns an on-chain revert into a local, readable rejection.
