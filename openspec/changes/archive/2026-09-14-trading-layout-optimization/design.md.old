...
*Cost control:* `/api/tickers` is built from `read.marketPrices.getAll()` + `read.marketContexts.getAll()` — **two upstream calls regardless of market count**, not one per market. `marketContexts.getAll()` was confirmed present in the installed SDK and carries `volume_24h`, `open_interest`, `previous_day_price` and `price_change_pct_24h`.
...
- **The chart is rebuilt on every candle refresh, and this change adds more work to each rebuild (volume series, crosshair subscription).** → The refresh cadence stays 15 s, and the added work is two `addSeries` calls, not a re-layout. Measured before and after on the Trade screen at 1440 px; if the rebuild exceeds 50 ms the volume series moves behind a check that skips it when the data is unchanged. Fixing the lifecycle properly is named in the README's "what's next" rather than smuggled into this change.
