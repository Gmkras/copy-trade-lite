...
/** Reads the signal from SQLite (not a component, so it may read the clock). */
function loadSignal(id: string) {
  const repo = signalsRepo();
  const signal = repo.getSignal(id);
  if (!signal) return null;
  return { signal: { ...signal, expired: isExpired(signal, Date.now()) }, copies: repo.listCopies(id) };
}

export default async function SignalPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const data = loadSignal(id);
  if (!data) notFound();
